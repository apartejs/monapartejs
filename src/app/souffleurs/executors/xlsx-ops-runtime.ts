...
function fakerValue(path: string, opts: Record<string, number | string>): string | number {
  ...
  switch (p) {
    case 'person.firstName':
      return pick(FR_FIRST);
    case 'person.lastName':
      return pick(FR_LAST);
    case 'person.fullName':
      return `${pick(FR_FIRST)} ${pick(FR_LAST)}`;
    case 'internet.email':
      return `${pick(FR_FIRST).toLowerCase()}.${pick(FR_LAST).toLowerCase()}@example.com`;
    case 'internet.username':
      return `${pick(FR_FIRST).toLowerCase()}${rint(1, 99)}`;
    case 'phone.number':
      return `0${rint(6, 7)} ${pad2(rint(0, 99))} ${pad2(rint(0, 99))} ${pad2(rint(0, 99))} ${pad2(rint(0, 99))}`;
    case 'location.city':
      return pick(FR_CITY);
    case 'location.zipCode':
      return String(rint(1000, 95000)).padStart(5, '0');
    case 'location.streetAddress':
      return `${rint(1, 200)} rue ${pick(FR_LAST)}`;
    case 'location.country':
      return 'France';
    case 'company.name':
      return pick(COMPANY);
    case 'commerce.productName':
      return `${pick(PRODUCT)} ${pick(['Pro', 'Plus', 'Standard', 'Premium'])}`;
    case 'commerce.price':
      return (rint(Number(opts['min'] ?? 10), Number(opts['max'] ?? 500)) + Math.random()).toFixed(
        2,
      );
    case 'finance.amount':
      return (
        rint(Number(opts['min'] ?? 100), Number(opts['max'] ?? 10000)) + Math.random()
      ).toFixed(2);
    case 'number.int':
      return rint(Number(opts['min'] ?? 0), Number(opts['max'] ?? 100));
    case 'number.float': {
      const v =
        Number(opts['min'] ?? 0) +
        Math.random() * (Number(opts['max'] ?? 100) - Number(opts['min'] ?? 0));
      const pr = Number(opts['precision'] ?? 2);
      return pr === 0 ? Math.round(v) : +v.toFixed(pr);
    }
    case 'string.uuid':
      return crypto.randomUUID();
    case 'database.mongodbObjectId':
      return Array.from({ length: 24 }, () => '0123456789abcdef'[rint(0, 15)]).join('');
    case 'lorem.sentence':
      return `${pick(PRODUCT)} ${pick(['à réviser', 'en cours', 'validé', 'à planifier'])}.`;
    case 'date.between':
    case 'date.future':
    case 'date.past':
    case 'date.recent': {
      const from = opts['from']
        ? new Date(String(opts['from'])).getTime()
...
function generateValues(gen: Generator, count: number): unknown[] {
  if (gen.faker) {
    const opts = { ...gen };
    delete opts.faker;
    delete (opts as Record<string, unknown>)['locale'];
    return Array.from({ length: count }, () =>
      fakerValue(gen.faker as string, opts as Record<string, number | string>),
    );
  }
  if (gen.pick) {
  ...
  for (const ref of expandRange(String(op['range'] || op['cell']))) {
    ...
    if (s['bold'] || s['italic'] || s['size'] || s['color']) {
      cell.font = {
        ...
        color: s['color'] ? { argb: argb(String(s['color'])) } : cell.font?.color,
      };
    }
    if (s['bgColor'])
      cell.fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: argb(String(s['bgColor'])) },
      };
...
function workbookPreview(wb: any): string {
  ...
  sheet.eachRow({ includeEmpty: false }, (row: Row, rowNumber: number) => {
    ...
    for (let c = 1; c <= Math.min(sheet.columnCount || maxCols, maxCols); c++) {
      ...
      if (val && typeof val === 'object') {
        const o = val as { formula?: string; result?: unknown; text?: string };
        val = o.formula ? `=${o.formula}` : (o.result ?? o.text ?? JSON.stringify(val));
      }
  ...
  if (!rowCount) return '';
  const more =
    (sheet.rowCount || 0) > maxRows
      ? `<div class="bp-gf__preview-empty">… ${sheet.rowCount - maxRows} lignes de plus</div>`
      : '';
...
async function buildWorkbook(ops: XlsxOp[], original?: ArrayBuffer | null): Promise<any> {
  ...
  for (const op of ops) {
    try {
      switch (op['op']) {
        case 'addWorksheet':
          sheet = wb.addWorksheet(String(op['name'] || `Sheet${wb.worksheets.length + 1}`));
          break;
        case 'selectSheet': {
          const t = wb.getWorksheet(String(op['name']));
          if (t) sheet = t;
          break;
        }
        case 'setCell':
          ensure().getCell(String(op['cell'] || op['range'])).value = op['value'];
          break;
        case 'addRow':
          ensure().addRow((op['values'] as unknown[]) || []);
          break;
        case 'fillRange': {
          const cells = expandRange(String(op['range']));
          const vals = op['values']
            ? (op['values'] as unknown[])
            : op['generator']
              ? generateValues(op['generator'] as Generator, cells.length)
              : null;
          if (vals)
            cells.forEach((c, i) => {
              ensure().getCell(c).value = vals[i] ?? null;
            });
          break;
        }
        case 'applyFormula': {
          for (const ref of expandRange(String(op['range']))) {
            ...
            ensure().getCell(ref).value = {
              formula: String(op['formula']).replace(/\{row\}/g, String(row)),
            };
          }
          break;
        }
        case 'setStyle':
          if (sheet) applyStyle(sheet, op);
          break;
        case 'mergeCells':
          if (sheet) sheet.mergeCells(String(op['range']));
          break;
        case 'setColumnWidth': {
          if (!sheet) break;
          ...
          if (widths)
            for (const [col, w] of Object.entries(widths))
              sheet.getColumn(col).width = Number(w) || 10;
          else if (op['column'])
            sheet.getColumn(String(op['column'])).width = Number(op['width']) || 10;
          break;
        }
        case 'setRowHeight':
          if (sheet && op['row'])
            sheet.getRow(Number(op['row'])).height = Number(op['height'] ?? 20);
          break;
        case 'setNumberFormat':
          if (sheet)
            for (const c of expandRange(String(op['range'])))
              sheet.getCell(c).numFmt = String(op['format'] || '0');
          break;
        case 'addBorder': {
          if (!sheet) break;
          const style = String(op['style'] || 'thin'),
            color = { argb: argb(String(op['color'] || '000000')) };
          const sides = (op['sides'] as string[]) || ['top', 'left', 'bottom', 'right'];
          const border: Record<string, unknown> = {};
          sides.forEach((s) => (border[s] = { style, color }));
          for (const c of expandRange(String(op['range'])))
            sheet.getCell(c).border = { ...sheet.getCell(c).border, ...border };
          break;
        }
        case 'freezePane':
          if (sheet)
            sheet.views = [
              {
                state: 'frozen',
                xSplit: Number(op['xSplit'] ?? 0),
                ySplit: Number(op['ySplit'] ?? 1),
              },
            ];
          break;
        case 'autoFilter':
          if (sheet) sheet.autoFilter = String(op['range'] || 'A1');
          break;
        case 'addChart':
...
export async function applyXlsxOps(
  ops: XlsxOp[],
  original?: ArrayBuffer | null,
): Promise<{ bytes: ArrayBuffer; preview: string }> {
...
export function extractCompleteOps(text: string): XlsxOp[] {
  ...
  while (i < s.length) {
    ...
    if (!complete) break;
    try {
      ops.push(JSON.parse(s.slice(i, j + 1)));
    } catch {
      break;
    }
    i = j + 1;
