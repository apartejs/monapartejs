    ...
    try {
      switch (op['op']) {
        case 'addParagraph': {
          ...
          if (align)
            pProps.alignment =
              (AlignmentType as Record<string, unknown>)[align.toUpperCase()] || AlignmentType.LEFT;
        ...
        case 'setHeading': {
          ...
          children.push(
            new Paragraph({
              heading: ((HeadingLevel as Record<string, unknown>)[levelKey] ||
                HeadingLevel.HEADING_1) as (typeof HeadingLevel)[keyof typeof HeadingLevel],
              ...
              (rowData) =>
                new TableRow({
                  children: (rowData || []).map(
                    (cellText) =>
                      new TableCell({
                        children: [new Paragraph({ children: buildRuns(cellText) })],
                      }),
                  ),
                }),
            );
  ...
  for (const op of ops) {
    ...
    switch (op['op']) {
      case 'setHeading': {
        ...
        parts.push(`<h${lvl}>${runsToHtml(op['text'] || '')}</h${lvl}>`);
        blocks++;
        break;
      }
      case 'addParagraph': {
        ...
        parts.push(`<p${style}>${runsToHtml(op['text'] ?? op['runs'] ?? '')}</p>`);
        blocks++;
        break;
      }
      case 'addQuote':
        parts.push(`<blockquote>${runsToHtml(op['text'] || '')}</blockquote>`);
        blocks++;
        break;
      case 'addTable': {
      ...
      case 'addHorizontalLine':
        ...
        break;
      case 'addPageBreak':
        parts.push('<hr class="bp-gf__pagebreak">');
        blocks++;
        break;
      case 'addBlankLine':
        parts.push('<p>&nbsp;</p>');
        break; // cosmetic, doesn't count
      default:
        break;
    }
  }
